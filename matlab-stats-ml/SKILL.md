...
description: >-
  MATLAB Statistics and Machine Learning Toolbox. Functions - fitcsvm, fitctree, fitcensemble, fitcknn, fitcnb, fitcnet, fitlm, fitglm, fitrgp, fitrensemble, lasso, kmeans, linkage, fitgmdist, dbscan, pca, tsne, factoran, ttest, ttest2, anova1, anovan, ranksum, chi2gof, fitdist, makedist, mle, ecdf, coxphfit, cvpartition, crossval, kfoldLoss, perfcurve, confusionchart, bayesopt, shapley, normalize. Tasks - compare two groups statistically, run a t-test or ANOVA, check if data is normally distributed, fit a distribution to data, classify patients or samples, train an SVM or random forest, predict outcomes with regression, find clusters in data, reduce dimensions with PCA or t-SNE, perform cross-validation, compute ROC curve and AUC, select important features, optimize hyperparameters, build a diagnostic model, analyze survival data, plot Kaplan-Meier curves, handle missing data, interpret model predictions with SHAP or LIME, compare multiple classifiers. Domains - biomarker discovery, clinical trial analysis, patient outcome prediction, diagnostic classification, medical data analysis, gene expression, proteomics, drug response, epidemiology, survival analysis, treatment comparison, statistical significance, p-value, confidence interval.
---
