...
Naive Bayes is a simple but effective Bayesian classifier that assumes feature independence.
...
[label, Posterior, Cost] = predict(Mdl, X(1:5, :));

fprintf('Predictions with posterior probabilities:\n');
for i = 1:5
    fprintf('Sample %d: %s (P=%.3f)\n', i, label{i}, max(Posterior(i,:)));
end
```
...
% 'normal' - Gaussian (default for continuous)
% 'kernel' - Kernel density estimation
% 'mvmn' - Multinomial (for discrete categorical)
% 'mn' - Multivariate multinomial

% Mixed distribution types
Mdl = fitcnb(X, Y, 'DistributionNames', {'normal', 'kernel', 'normal', 'kernel'});

% All kernel for non-Gaussian features
Mdl_kernel = fitcnb(X, Y, 'DistributionNames', 'kernel');

% Compare performance
cv_normal = crossval(fitcnb(X, Y));
cv_kernel = crossval(fitcnb(X, Y, 'DistributionNames', 'kernel'));

fprintf('CV Loss (Normal): %.4f\n', kfoldLoss(cv_normal));
fprintf('CV Loss (Kernel): %.4f\n', kfoldLoss(cv_kernel));
```

### Kernel Settings

```matlab
% Customize kernel for each feature
Mdl = fitcnb(X, Y, ...
    'DistributionNames', 'kernel', ...
    'Kernel', 'normal', ...           % Kernel type: normal, box, epanechnikov, triangle
    'Width', 'auto');                  % Bandwidth: 'auto' or numeric

% Custom bandwidths per feature
Mdl = fitcnb(X, Y, ...
    'DistributionNames', 'kernel', ...
    'Width', [0.5, 0.3, 1.0, 0.8]);   % Per-feature bandwidths
```

### Prior Probabilities

```matlab
% Default: empirical class frequencies
Mdl_empirical = fitcnb(X, Y);

% Uniform prior
Mdl_uniform = fitcnb(X, Y, 'Prior', 'uniform');

% Custom prior (e.g., class imbalance correction)
customPrior = [0.5, 0.3, 0.2];  % Must sum to 1
Mdl_custom = fitcnb(X, Y, 'Prior', customPrior);

% View priors
disp('Class priors:');
disp(Mdl_empirical.Prior);
```

### Feature Extraction from Naive Bayes

```matlab
% Get class-conditional distributions
Mdl = fitcnb(X, Y);
...
for i = 1:length(classNames)
    fprintf('Class: %s\n', classNames{i});
    for j = 1:size(X, 2)
        mu = Mdl.DistributionParameters{i, j}(1);
        sigma = Mdl.DistributionParameters{i, j}(2);
        fprintf('  Feature %d: mu=%.2f, sigma=%.2f\n', j, mu, sigma);
    end
    fprintf('\n');
end
```
...
### Analytical Bayesian Regression

```matlab
function [w_mean, w_cov, sigma2] = bayesianLinearRegression(X, y, alpha, beta)
    % Bayesian linear regression with conjugate priors
    % Prior: w ~ N(0, alpha^-1 * I)
    % Likelihood: y | w ~ N(X*w, beta^-1 * I)
    %
    % alpha: prior precision
    % beta: noise precision (1/variance)

    [n, d] = size(X);

    % Posterior covariance
    S_inv = alpha * eye(d) + beta * (X' * X);
    w_cov = inv(S_inv);

    % Posterior mean
    w_mean = beta * w_cov * X' * y;
...
% Example
n = 100;
X = [ones(n, 1), randn(n, 2)];  % With intercept
w_true = [1; 2; -1];
y = X * w_true + 0.5 * randn(n, 1);

% Fit
alpha = 0.1;  % Prior precision (weak prior)
beta = 4;     % Noise precision (known noise variance = 0.25)

[w_mean, w_cov, sigma2] = bayesianLinearRegression(X, y, alpha, beta);

fprintf('True weights: %s\n', mat2str(w_true', 4));
fprintf('Posterior mean: %s\n', mat2str(w_mean', 4));
fprintf('Posterior std: %s\n', mat2str(sqrt(diag(w_cov))', 4));
```

### Predictive Distribution

```matlab
function [mu_pred, sigma_pred] = bayesianPredict(X_new, w_mean, w_cov, sigma2)
    % Predictive distribution for new points

    % Predictive mean
    mu_pred = X_new * w_mean;

    % Predictive variance (includes both parameter and noise uncertainty)
    sigma_pred = zeros(size(X_new, 1), 1);
    for i = 1:size(X_new, 1)
        sigma_pred(i) = X_new(i, :) * w_cov * X_new(i, :)' + sigma2;
    end
    sigma_pred = sqrt(sigma_pred);
end

% Predict
X_new = [ones(10, 1), randn(10, 2)];
[mu_pred, sigma_pred] = bayesianPredict(X_new, w_mean, w_cov, sigma2);

% 95% credible intervals
ci_lower = mu_pred - 1.96 * sigma_pred;
ci_upper = mu_pred + 1.96 * sigma_pred;

fprintf('Predictions with 95%% CI:\n');
for i = 1:min(5, length(mu_pred))
    fprintf('  y = %.3f [%.3f, %.3f]\n', mu_pred(i), ci_lower(i), ci_upper(i));
end
```
